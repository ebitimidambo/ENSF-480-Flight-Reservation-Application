...
public class AdminAircraftsUI extends JPanel {
	private User user;
	...
	public AdminAircraftsUI(User user) {
        this.user = user;
        this.factory = new AdminGuiFactory(user);

        this.header = factory.createHeader();
        if (header instanceof AdminHeader adminHeader) {
        	adminHeader.setActivePage(AdminPage.AIRCRAFT);
            adminHeader.setRoutesListener(e -> openNewUI(AdminNavigation.ROUTES));
    ...
    private void setupUI() {
        ...
        AdminBody.setMode(AdminBody.AIRCRAFT_MODE);
